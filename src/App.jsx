...
import { Upload, FileSpreadsheet, Download, Share2, AlertTriangle, Moon, Sun, Search, Calendar, CheckCircle, XCircle, Lock, Flame, Filter, Menu, CloudUpload, Plus, X } from 'lucide-react';
...
function App() {
  ...
  const salvarNaNuvem = async () => {
    ...
    const nomeEscala = prompt("Dê um nome para essa escala (ex: Dezembro 2025):");
    if (!nomeEscala) return;
    setLoading(true);
    const { data, error } = await supabase.from('escalas').insert([{ nome: nomeEscala, dados: escala }]).select();
    setLoading(false);
    if (error) { console.error(error); showToast("Erro ao salvar.", "error"); } 
    else { 
        const link = `${window.location.origin}/share?id=${data[0].id}`; 
        navigator.clipboard.writeText(link); 
        alert(`Salvo! Link copiado:\n${link}`); 
    }
  };
  ...
  const confirmExport = () => {
    const escalaFiltrada = escala.filter(slot => {
        let areaKey = slot.AreaOriginal || 'OUTROS';
        ...
        if(slot.Funcao.includes('PROJEÇÃO')) areaKey = 'PROJEÇÃO';
        if(slot.Funcao.includes('ILUMINAÇÃO')) areaKey = 'ILUMINAÇÃO';
        return ministeriosExportacao.includes(areaKey);
    });
    ...
    if (exportType === 'pdf') exportarPDF(escalaFiltrada);
    if (exportType === 'excel') exportarExcel(escalaFiltrada);
    if (exportType === 'ics') exportarICS(escalaFiltrada);
    if (exportType === 'whatsapp') {
        copiarWhatsApp(escalaFiltrada)
  ...
  const adicionarPessoaManual = () => {
    ...
    if (adicionaisManuais.includes(novoNome)) { showToast("Já existe!", "error"); return; }
    setAdicionaisManuais([...adicionaisManuais, novoNome]);
    setNovoNome("");
    ...
    <div className={`w-full min-h-screen flex flex-col transition-colors duration-300 ${bgClass} font-sans`}>
      <header className={`w-full px-4 md:px-8 py-3 shadow-md ${isDark ? 'bg-gray-800/95 border-gray-700' : 'bg-white/95 border-gray-300'} backdrop-blur-md flex justify-between items-center sticky top-0 z-50 border-b`}>
        <div className="flex items-center gap-3">
          <img src="/favicon.png" alt="ChorusApp" className="w-9 h-9 md:w-10 md:h-10 object-contain drop-shadow-sm" />
        ...
        <div className="flex items-center gap-2">
            <button onClick={toggleTheme} className={`p-2 rounded-full border transition-all ${isDark ? 'bg-gray-700 border-gray-600' : 'bg-gray-100 border-gray-300'}`}>
            ...
            <div className={`w-full max-w-xl p-6 md:p-10 rounded-3xl ${cardClass} transition-all`}>
              ...
              <div className="space-y-6">
                <div className="flex items-center justify-center w-full group">
                  <label className={`flex flex-col items-center justify-center w-full h-40 md:h-52 border-2 border-dashed rounded-3xl cursor-pointer transition-all duration-300 ${isDark ? 'border-gray-600 bg-gray-800/50 hover:bg-gray-800 hover:border-blue-500' : 'border-gray-400 bg-gray-50 hover:bg-white hover:border-blue-600 hover:shadow-lg'}`}>
                    <div className="flex flex-col items-center justify-center pt-5 pb-6"><div className="p-4 bg-blue-100 dark:bg-gray-700 rounded-full mb-3 group-hover:scale-110 transition-transform shadow-inner"><Upload className="w-6 h-6 md:w-8 md:h-8 text-blue-600 dark:text-blue-400" /></div><p className="mb-1 text-sm font-bold text-gray-600 dark:text-gray-400 group-hover:text-blue-600 transition-colors">Clique para enviar</p><p className="text-xs text-gray-500 bg-gray-200 dark:bg-gray-700 px-3 py-1 rounded-full border border-gray-300 dark:border-gray-600">.XLSX</p></div>
            ...
            <div className={`py-3 px-4 -mx-3 md:mx-0 mb-6 border-b shadow-sm flex flex-col md:flex-row gap-3 justify-between items-center transition-colors rounded-2xl ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-300'}`}>
              <div className="relative w-full md:w-96 flex gap-2">
                <div className="relative flex-1">
                    ...
                    <input type="text" placeholder="Buscar..." value={filtro} onChange={(e) => setFiltro(e.target.value)} className={`w-full pl-10 pr-4 py-2.5 rounded-xl border focus:ring-2 focus:ring-blue-500 outline-none transition-all font-medium text-sm md:text-base ${isDark ? 'bg-gray-700 border-gray-600 text-white placeholder-gray-400' : 'bg-gray-50 border-gray-300 text-gray-800 placeholder-gray-500 focus:bg-white'}`} />
                </div>
              ...
              <div className="flex flex-wrap items-center justify-center gap-2 w-full md:w-auto">
                ...
                <ExportBtn icon={<CloudUpload size={16}/>} label="Salvar" onClick={salvarNaNuvem} color="purple" />
      ...
      {showManualAddModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className={`w-full max-w-sm p-6 rounded-3xl shadow-2xl ${isDark ? 'bg-gray-800' : 'bg-white'} animate-bounce-in`}>
                <h3 className="text-xl font-bold mb-4">Adicionar Pessoa</h3>
                <input type="text" placeholder="Nome" className={`w-full p-3 rounded-xl border mb-4 outline-none focus:ring-2 focus:ring-blue-500 ${isDark ? 'bg-gray-700 border-gray-600' : 'bg-gray-50 border-gray-300'}`} value={novoNome} onChange={(e) => setNovoNome(e.target.value)}/>
                <div className="flex gap-3"><button onClick={() => setShowManualAddModal(false)} className="flex-1 py-3 rounded-xl font-bold text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-700">Cancelar</button><button onClick={adicionarPessoaManual} className="flex-1 py-3 rounded-xl font-bold text-white bg-blue-600 hover:bg-blue-700">Adicionar</button></div>
            </div>
        </div>
      )}
      ...
      {showExportModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className={`w-full max-w-md p-6 rounded-3xl shadow-2xl ${isDark ? 'bg-gray-800' : 'bg-white'} animate-bounce-in`}>
                <div className="flex justify-between items-center mb-6"><h3 className="text-xl font-bold">Exportar</h3><button onClick={() => setShowExportModal(false)}><X size={24} className="text-gray-400"/></button></div>
                <div className="grid grid-cols-2 gap-3 mb-8">{Object.keys(MINISTERIOS_DEFAULT).map(min => (<label key={min} className={`flex items-center gap-2 p-3 rounded-xl border cursor-pointer transition-all ${ministeriosExportacao.includes(min) ? 'bg-blue-50 border-blue-500 text-blue-700 dark:bg-blue-900/30' : 'border-gray-200 dark:border-gray-700'}`}><input type="checkbox" className="w-5 h-5 rounded text-blue-600 focus:ring-blue-500" checked={ministeriosExportacao.includes(min)} onChange={(e) => {if(e.target.checked) setMinisteriosExportacao([...ministeriosExportacao, min]); else setMinisteriosExportacao(ministeriosExportacao.filter(m => m !== min));}}/><span className="text-sm font-bold">{min}</span></label>))}</div>
                <button onClick={confirmExport} className="w-full py-4 rounded-xl font-bold text-white bg-blue-600 hover:bg-blue-700 shadow-lg shadow-blue-500/30">Confirmar</button>
            </div>
        </div>
      )}
